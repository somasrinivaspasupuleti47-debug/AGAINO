...
exports.emailService = {
    async sendOtpEmail(to, otp) {
        ...
        try {
            await transporter.sendMail({
                from: `"AGAINO" <${env_1.env.SMTP_USER}>`,
                to,
    ...
    async notifyAdminListingPublished(listingId, title, sellerId) {
        if (!env_1.env.SMTP_HOST || !env_1.env.ADMIN_EMAIL)
            return;
        try {
            await transporter.sendMail({
                from: `"AGAINO" <${env_1.env.SMTP_USER}>`,
          ...
          <div style="font-family:sans-serif;max-width:480px;margin:auto;padding:32px;border-radius:12px;border:1px solid #eee">
            ...
            <a href="${env_1.env.FRONTEND_URL}/listings/${listingId}" style="display:inline-block;background:#f97316;color:white;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:bold;margin-top:16px">View Listing</a>
          </div>
        `,
            });
    ...
    async notifyAdminListingSold(listingId, title, sellerId) {
        if (!env_1.env.SMTP_HOST || !env_1.env.ADMIN_EMAIL)
            return;
        try {
            await transporter.sendMail({
